...
export default function AppHeader() {
  ...
  const handleCreateRFI = () => {
    if (selectedProjectId) {
      navigate(`/subcontractor-dashboard/projects/${selectedProjectId}?tab=rfis&action=create`);
    } else if (projects.length > 0) {
      navigate(`/subcontractor-dashboard/projects/${projects[0].id}?tab=rfis&action=create`);
    } else {
      navigate("/subcontractor-dashboard/rfi");
    }
  };

  const handleCreateChangeOrder = () => {
    if (selectedProjectId) {
      navigate(`/subcontractor-dashboard/projects/${selectedProjectId}?tab=change-orders&action=create`);
    } else if (projects.length > 0) {
      navigate(`/subcontractor-dashboard/projects/${projects[0].id}?tab=change-orders&action=create`);
    } else {
      navigate("/subcontractor-dashboard/change-orders");
    }
  };

  const handleCreateDailyLog = () => {
    if (selectedProjectId) {
      navigate(`/subcontractor-dashboard/projects/${selectedProjectId}?tab=daily-logs&action=create`);
    } else if (projects.length > 0) {
      navigate(`/subcontractor-dashboard/projects/${projects[0].id}?tab=daily-logs&action=create`);
    } else {
      navigate("/subcontractor-dashboard/daily-logs");
    }
  };

  const handleCreateMilestone = () => {
    if (selectedProjectId) {
      navigate(`/subcontractor-dashboard/projects/${selectedProjectId}?tab=schedule&action=create`);
    } else {
      navigate("/subcontractor-dashboard/schedule");
    }
  };
              ...
              projects.map((project) => (
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    ...
                    navigate(`/subcontractor-dashboard/projects/${project.id}`);
                  }}
                  className="flex items-center justify-between"
                >
      ...
      <div className="flex items-center gap-2">
                  ...
                  <DropdownMenuItem
                    ...
                    onClick={() => {
                      if (notification.projectId) {
                        navigate(`/subcontractor-dashboard/projects/${notification.projectId}`);
                      }
                    }}
                  >
            ...
            {notifications.length > 0 && (
              <>
                ...
                <DropdownMenuItem
                  ...
                  onClick={() => navigate("/subcontractor-dashboard/rfi")}
                >
                  View all activity
                </DropdownMenuItem>
              </>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => navigate("/subcontractor-dashboard/settings")}>
