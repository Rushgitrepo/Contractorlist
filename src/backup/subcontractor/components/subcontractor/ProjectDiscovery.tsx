/**
 * SC ProjectDiscovery Component
 * Find projects and submit bids - with real API integration
 * Mirrors GC ProjectDiscovery functionality for Subcontractors
 */
...
const ProjectDiscovery = () => {
  ...
  const handleOpenBidModal = () => {
    ...
    setBidItems([{ name: '', description: '', price: 0 }]);
    setBidNotes('');
    setBidStartDate('');
    setBidEndDate('');
    setBidCompanyHighlights('');
    setBidRelevantExperience('');
    setBidCredentials('');
  ...
  const handleSubmitBid = async () => {
    ...
    try {
      ...
      // Validate items
      const validItems = bidItems.filter(item => item.name.trim() && item.price > 0);
      if (validItems.length === 0) {
        setBidFormError('Please add at least one line item with a name and price.');
        toast({
          title: 'Validation Error',
          description: 'Please add at least one line item with a name and price.',
          variant: 'destructive',
        });
      ...
      // Create the bid
      const bid = await scApi.createBid({
        ...
        estimatedStartDate: bidStartDate || undefined,
        estimatedEndDate: bidEndDate || undefined,
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        setIsLoading(true);
        const data = await scApi.getProjectDiscovery();
        setProjects(data);
      } catch (error) {
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter(p => {
      ...
      const getBudgetValue = (b: string) => {
        const numeric = parseFloat(b?.replace(/[^0-9.]/g, '') || '0');
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-black">Find Projects</h2>
                <Badge variant="outline" className="text-[9px] border-none text-gray-500 bg-gray-100 dark:bg-white/5 px-3 py-1 uppercase">
                  {isLoading ? 'Loading...' : `${filteredProjects.length} Projects Available`}
                </Badge>
              </div>
              ...
              <div className={cn("grid gap-8", viewMode === 'card' ? "grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3" : "grid-cols-1")}>
                    ...
                    <CardContent className="p-8">
                      <div className="flex flex-col h-full gap-6">
                        <div className="flex justify-between items-start">
                          <Badge className="bg-gray-100 dark:bg-white/10 text-gray-900 dark:text-white border-none font-black text-[9px] h-5 tracking-widest uppercase">{p.source || 'Marketplace'}</Badge>
                          <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{p.posted || 'Recently'}</span>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-[11px] font-bold text-gray-400">
                            <span className="flex items-center gap-1"><MapPin size={12} className="text-yellow-600" /> {p.location} {p.distance && `(${p.distance})`}</span>
                            <span>•</span>
                        ...
                        <div className="pt-6 border-t border-gray-100 dark:border-white/5 flex items-center justify-between">
                          ...
                          <Button className="h-10 px-6 rounded-xl bg-black dark:bg-yellow-500 text-white dark:text-black font-black uppercase text-[10px] tracking-widest">View & Bid</Button>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                ...
                {filteredProjects.length === 0 && (
                  <div className="col-span-full py-24 flex flex-col items-center justify-center text-center bg-gray-50 dark:bg-white/[0.02] rounded-2xl border-2 border-dashed border-gray-200 dark:border-white/5">
                    ...
                    <h4 className="text-2xl font-black tracking-tight mb-2">No Projects Found</h4>
                    <p className="text-gray-500 max-w-sm font-bold text-sm">No projects match your criteria. Try adjusting your filters.</p>
                  </div>
                )}
              </div>
            )}
          ...
          {selectedProject && (
            <div className="flex flex-col">
              <div className="bg-gray-100/50 dark:bg-black/30 p-8 border-b border-gray-100 dark:border-white/5">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex gap-2">
                    <Badge className="bg-gray-100 dark:bg-white/10 text-gray-900 dark:text-white border-none font-black text-[9px] uppercase tracking-widest px-3">{selectedProject.source || 'Marketplace'}</Badge>
                    <Badge className="bg-yellow-400 text-black border-none font-black text-[9px] uppercase tracking-widest px-3">Verified</Badge>
                  </div>
                ...
                <div className="flex items-center gap-4 text-xs font-bold text-gray-500">
                  ...
                  {selectedProject.nigpCode && (
                    <>
                      <span>•</span>
                      <span className="flex items-center gap-1.5 border-l border-gray-300 dark:border-white/10 pl-4"><Tag size={14} className="text-gray-900 dark:text-white" /> NIGP: {selectedProject.nigpCode}</span>
                    </>
                  )}
                </div>
              </div>
              ...
              <div className="p-8 space-y-8 max-h-[60vh] overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ...
                  <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5">
                    ...
                    <p className="text-sm font-black">{selectedProject.sqft || 'N/A'}</p>
                  </div>
                  <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5">
                  ...
                  <div className="p-4 rounded-2xl bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5">
                    ...
                    <p className="text-sm font-black uppercase text-yellow-600 dark:text-yellow-500">{selectedProject.category || selectedProject.projectType}</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400">Project Description</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed font-semibold">
                    {selectedProject.description || 'No description available.'}
                  </p>
                </div>

                {selectedProject.trades && selectedProject.trades.length > 0 && (
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400">Required Trades</h4>
                    <div className="flex flex-wrap gap-2">
                      {selectedProject.trades.map((t: string) => (
                        <Badge key={t} variant="secondary" className="bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-300 border-none text-[9px] font-bold">{t}</Badge>
                      ))}
                    </div>
                  </div>
                )}
              ...
              <div className="p-8 bg-gray-100 dark:bg-black/30 border-t border-gray-100 dark:border-white/5 flex gap-4">
                <Button onClick={() => setShowDetailsModal(false)} variant="outline" className="flex-1 h-12 rounded-xl text-[10px] font-black uppercase tracking-widest dark:border-white/10">Close</Button>
                <Button onClick={handleOpenBidModal} className="flex-[2] h-12 rounded-xl bg-yellow-400 hover:bg-yellow-500 text-black font-black uppercase text-[10px] tracking-widest shadow-lg">
                  <Briefcase className="w-4 h-4 mr-2" /> Submit Proposal
                </Button>
              </div>
            </div>
          )}
            ...
            <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar">
              {bidFormError && (
                <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/30 rounded-xl">
                  <p className="text-sm text-red-600 dark:text-red-400 font-semibold">{bidFormError}</p>
                </div>
              )}
              ...
              <div className="p-6 rounded-3xl bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-100 dark:border-yellow-900/20 space-y-6">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-[11px] font-black uppercase tracking-tighter text-yellow-800 dark:text-yellow-500">Company Credentials (Optional)</h4>
                </div>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Proposal Summary / Notes</Label>
                <Textarea
                  placeholder="Summarize your proposal and methodology..."
                  value={bidNotes}
